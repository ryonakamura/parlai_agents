...
class MemN2NAgent(Agent, chainer.Chain):
    ...
    def __init__(self, opt, shared=None):
        ...
        if not shared:
            ...
            # Layer-wise (RNN-like) weight tying
                ...
                super(Agent, self).add_link('H', L.Linear(hs, hs))
            ...
            # No weight tying
            if self.weight_tying == 'Nothing':
                super(Agent, self).add_link('B', L.EmbedID(vs, hs))
                super(Agent, self).add_link('W', L.EmbedID(vs, hs))
            ...
            super(Agent, self).add_link('double', L.Linear(hs, hs*2))
    ...
    def _position_encoding(self, sentence):
        ...
        if sentence.data.ndim == 2: # sentence is q
            bs, ss = sentence.data.shape[:]
            sentence = sentence.reshape((bs, 1, ss))
        bs, ms, ss = sentence.data.shape[:]
        hs = self.hidden_size
        # Make k
        ...
        for _x, x in enumerate(sentence.data):
    ...
    def _embedding(self, embed, sentence, l=None): # batch x memory x sequence
        # No memory size rank when sentences is q (question)
        e = embed(sentence) # batch x memory x sequence x hidden
        
        # Position Encoding
        if self.use_position_encoding:
            if e.data.ndim == 3: # sentence is q
                bs, ss, hs = e.data.shape[:]
                e = e.reshape((bs, 1, ss, hs))
            e = l * e # batch x memory x sequence x hidden
            if e.data.shape[1] == 1: # sentence is q
                e = e.reshape((bs, ss, hs))

        # With negative axis, the same rank of x or q can be specified for sentences.
